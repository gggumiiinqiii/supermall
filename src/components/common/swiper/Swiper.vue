...
export default {
  ...
  data(){
    return {
      currIndex:1,//当前图片的index
      swiperStyle:{},//style样式
      slideWidth:null,//一张slide的宽度
      moveTimer:null,
      startPos:0,
      distance:0,//滚动距离
      slideCount:4//总共的图片数量
    };
  },
  props:{
    moveRatio:{
  ...
  methods:{
    ...
    initDom(){
      ...
      this.slideCount = slides.length;//页数
      console.log(this.slideCount)
      this.slideWidth = swiperEl.offsetWidth;//整数padding+border+width的值
